...
// define default opengl screen size
...
void storePosition(const nite::SkeletonJoint joint){
	FILE *fp;
	char fileName[20] = "";
	switch (joint.getType()){
		case nite::JOINT_HEAD:
			sprintf(fileName, "head.dat");
			break;
		case nite::JOINT_TORSO:
			sprintf(fileName, "torse.dat");
			break;
		case nite::JOINT_NECK:
			sprintf(fileName, "neck.dat");
			break;
		case nite::JOINT_LEFT_ELBOW:
			sprintf(fileName, "left_elbow.dat");
			break;
		case nite::JOINT_RIGHT_ELBOW:
			sprintf(fileName, "right_elbow.dat");
			break;
		case nite::JOINT_RIGHT_HIP:
			sprintf(fileName, "right_hip.dat");
			break;
		case nite::JOINT_LEFT_HIP:
			sprintf(fileName, "left_hip.dat");
			break;
		case nite::JOINT_RIGHT_SHOULDER:
			sprintf(fileName, "right_shoulder.dat");
			break;
		case nite::JOINT_LEFT_SHOULDER:
			sprintf(fileName, "left_shoulder.dat");
			break;
		case nite::JOINT_LEFT_FOOT:
			sprintf(fileName, "left_foot.dat");
			break;
		case nite::JOINT_RIGHT_FOOT:
			sprintf(fileName, "right_foot.dat");
			break;
		case nite::JOINT_RIGHT_KNEE:
			sprintf(fileName, "right_knee.dat");
			break;
		case nite::JOINT_LEFT_KNEE:
			sprintf(fileName, "left_knee.dat");
			break;
		case nite::JOINT_LEFT_HAND:
			sprintf(fileName, "left_hand.dat");
			break;
		case nite::JOINT_RIGHT_HAND:
			sprintf(fileName, "right_hand.dat");
			break;
		default:
			break;
	}
	...
	if(fp == NULL){
		printf("file is not successfully opened");
		exit(0);
	}
	char output[50] = "";
...
void storeAllJoints(const nite::UserData& userData){
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_HEAD));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_TORSO));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_NECK));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_RIGHT_SHOULDER));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_LEFT_HIP));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_RIGHT_HIP));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_LEFT_ELBOW));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_RIGHT_ELBOW));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_LEFT_KNEE));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_RIGHT_KNEE));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_LEFT_HAND));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_RIGHT_HAND));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_LEFT_FOOT));
	storePosition(userData.getSkeleton().getJoint(nite::JOINT_RIGHT_FOOT));
}
...
void DrawLimb(nite::UserTracker* pUserTracker, const nite::SkeletonJoint& joint1, const nite::SkeletonJoint& joint2, int color)
{
	...
	// convert 3D dimension coordinates into 2D
	...
	// fit coordinates into windows size
	coordinates[0] *= GL_WIN_SIZE_X/g_nXRes;
	coordinates[1] *= GL_WIN_SIZE_Y/g_nYRes;
	coordinates[3] *= GL_WIN_SIZE_X/g_nXRes;
	coordinates[4] *= GL_WIN_SIZE_Y/g_nYRes;

	// decide on the color of limb
	/// if confidence is too small, no limb will be drawn
...
// draw user skeleton based on user data
...
// display user shade and user skeleton
void SampleViewer::Display()
{
	nite::UserTrackerFrameRef userTrackerFrame;
	openni::VideoFrameRef depthFrame;

	// read user tracker frame
	...
	{
		// pointer to the first depth pixel
...
// 					}
	...
	for (int i = 0; i < users.getSize(); ++i)
	{
		...
		else if (!user.isLost())
		{
			...
			if ((users[i].getSkeleton().getState() == nite::SKELETON_TRACKED || users[i].getSkeleton().getState() == nite::SKELETON_TRACKED ) && g_drawSkeleton)
			{
